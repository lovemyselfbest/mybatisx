...
public final class TypeResolver {
    ...
    public TypeResolver where(Type formal, Type actual) {
        Map<TypeVariableKey, Type> mappings = new HashMap<TypeVariableKey, Type>();
    ...
    private static void populateTypeMappings(
        ...
        new TypeVisitor() {
            ...
            void visitWildcardType(WildcardType fromWildcardType) {
                ...
                if (!(fromUpperBounds.length == toUpperBounds.length
                        && fromLowerBounds.length == toLowerBounds.length)) {
                    throw new IllegalArgumentException(String.format("Incompatible type: " +
                            "%s vs. %s", fromWildcardType, to));
                }
            ...
            void visitParameterizedType(ParameterizedType fromParameterizedType) {
                ...
                if (!fromParameterizedType.getRawType().equals(toParameterizedType.getRawType())) {
                    throw new IllegalArgumentException(String.format("Inconsistent raw type: %s vs. %s",
                            fromParameterizedType, to));
                }
                Type[] fromArgs = fromParameterizedType.getActualTypeArguments();
                Type[] toArgs = toParameterizedType.getActualTypeArguments();
                if (fromArgs.length != toArgs.length) {
                    throw new IllegalArgumentException(String.format("%s not compatible with %s",
            ...
            void visitGenericArrayType(GenericArrayType fromArrayType) {
                ...
                if (componentType == null) {
                    throw new IllegalArgumentException(String.format("%s is not an array type.", to));
                }
    ...
    private static class TypeTable {
        ...
        TypeTable() {
            this.map = new HashMap<TypeVariableKey, Type>();
        }
        ...
        final TypeTable where(Map<TypeVariableKey, ? extends Type> mappings) {
            Map<TypeVariableKey, Type> newMap = new HashMap<TypeVariableKey, Type>();
            newMap.putAll(map);
            for (Map.Entry<TypeVariableKey, ? extends Type> mapping : mappings.entrySet()) {
                ...
                if (variable.equalsType(type)) {
                    throw new IllegalArgumentException(String.format("Type variable %s bound to itself", variable));
                }
                newMap.put(variable, type);
            }
            return new TypeTable(newMap);
        }
    ...
    private static final class TypeMappingIntrospector extends TypeVisitor {
        ...
        private final Map<TypeVariableKey, Type> mappings = new HashMap<TypeVariableKey, Type>();
        ...
        static Map<TypeVariableKey, Type> getTypeMappings(
            ...
            Map<TypeVariableKey, Type> copyMap = new HashMap<TypeVariableKey, Type>(introspector.mappings);
        ...
        Type capture(Type type) {
            ...
            if (type instanceof WildcardType) {
                ...
                if (lowerBounds.length == 0) { // ? extends something changes to capture-of
                    ...
                    //+ Joiner.on('&').join(upperBounds); //TODO
                    return Types.newArtificialTypeVariable(
